...
bool Kinect::run(){
	...
	boost::function<void(const boost::shared_ptr<Image>& image)> f_image =
		[this](const boost::shared_ptr<Image>& image) 
	{ 
		Mat im = Mat(image->getHeight(), image->getWidth(), CV_8UC3);
	...
	case 0:
		type = "SHOT";
		f_cloud = [this](const PointCloudType::ConstPtr& cloud) { _3D_Matching_SHOT(cloud); };
		if (!prepareModelSHOT())
			return false;
		break;
	case 1:
		type = "PPF";
		f_cloud = [this](const PointCloudType::ConstPtr& cloud) { _3D_Matching_PPF(cloud); };;
		if (!prepareModelPPF())
			return false;
		break;
	case 2:
		type = "B2BTL_MEAM";
		f_cloud = [this](const PointCloudType::ConstPtr& cloud) { _3D_Matching_B2BTL_MEAM(cloud); };;
		if (!prepareModelB2BTL_MEAM())
			return false;
		break;
	default:
		break;
	}
	std::cout << "Descriptor type: " << type << endl;
...
bool Kinect::isRunning() {
	return (!customViewer.viewer->wasStopped());
}

bool Kinect::loadModel() {
...
bool Kinect::prepareModelSHOT()
{
	...
	//------------------------------------------------------ Visualization --------------------------------------------------------------------
	customViewer.init();
	customViewer.addCloud(scene, "scene", customViewer.v1);
	customViewer.addCloud(scene, "scene_cloud", customViewer.v2);
...
bool Kinect::prepareModelB2BTL_MEAM()
{
	//  Load model cloud
	if (!loadModel())
		return false;
...
void Kinect::_3D_Matching_SHOT(const PointCloudType::ConstPtr &cloud)
{
	mtx.lock();
	...
	*scene = *statistical_filtered_scene; // get the preprocessed scene
	...
	if (!customViewer.viewer->wasStopped()) {
		customViewer.updateCloud(scene, "scene");
		customViewer.updateCloud(scene, "scene_cloud");
		customViewer.updateCloud(scene_keypoints, "scene_keypoints");
		customViewer.updateCloud(off_model_good_kp, "model_good_keypoints");
		customViewer.updateCloud(scene_good_kp, "scene_good_keypoints");
	}
	...
	// -----------------------Save scene -------------------------
	//pcl::io::savePCDFile("scene2.pcd", *scene, true); // Binary format
...
void Kinect::_3D_Matching_PPF(const PointCloudType::ConstPtr &cloud)
{
	mtx.lock();
...
void Kinect::_3D_Matching_B2BTL_MEAM(const PointCloudType::ConstPtr &cloud)
{
	mtx.lock();
	tt.tic();
